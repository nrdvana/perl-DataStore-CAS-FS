...
use_ok('DataStore::CAS::FS::Dir::Minimal') || BAIL_OUT;
...
my @expected= (
	{ type => 'file',     name => 'a', size => undef, ref => '0000',   },
	{ type => 'file',     name => 'b', size => undef, ref => '1111',   },
	{ type => 'symlink',  name => 'c', size => undef, ref => 'fedcba', },
	{ type => 'blockdev', name => 'd', size => undef, ref => '1234',   },
	{ type => 'chardev',  name => 'e', size => undef, ref => '4321',   },
...
my $ser= DataStore::CAS::FS::Dir::Minimal->SerializeEntries(\@entries, \%metadata);
...
isa_ok( my $dir= DataStore::CAS::FS::Dir->new($file), 'DataStore::CAS::FS::Dir::Minimal', 'created dir object');

for my $e (@expected) {
	subtest $e->{name} => sub {
		ok( my $entry= $dir->get_entry($e->{name}), "find entry" );
		for my $k (keys %$e) {
			is( $entry->$k(), $e->{$k}, "match field $k" )
				or diag Dumper($entry);
		}
		done_testing;
	};
}
