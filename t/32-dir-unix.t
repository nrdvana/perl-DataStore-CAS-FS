...
use_ok('File::CAS::Dir::Unix') || BAIL_OUT;
...
my @expected= (
	{ type => 'file',     name => 'a', hash => '0000', size => 10, linkTarget => undef, device => undef },
	{ type => 'file',     name => 'b', hash => '1111', size => 10, linkTarget => undef, device => undef },
	{ type => 'symlink',  name => 'c', hash => undef, size => 10, linkTarget => 'fedcba', device => undef },
	{ type => 'blockdev', name => 'd', hash => undef, size => 10000, linkTarget => undef, device => '1234' },
	{ type => 'chardev',  name => 'e', hash => undef, size => 0, linkTarget => undef, device => '4321' },
	{ type => 'pipe',     name => 'f', hash => undef, size => 1, linkTarget => undef, device => undef },
	{ type => 'socket',   name => 'g', hash => undef, size => 1, linkTarget => undef, device => undef },
);

my $sto= new_ok('File::CAS::Store::Virtual', [], 'create temp store');

my $ser= File::CAS::Dir::Unix->SerializeEntries(\@entries, \%metadata);
...
isa_ok( my $dir= File::CAS::Dir->new($file), 'File::CAS::Dir::Unix', 'created dir object');
