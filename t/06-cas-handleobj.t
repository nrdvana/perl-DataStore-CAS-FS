...
my $h= new_ok( 'DataStore::CAS::Handle', [ $self, { char => 'x' } ] );

is( $h->_store, $self, '_store attr' );
is_deeply( $h->_data, { char => 'x' }, '_data attr' );
...
is( $h->other_method('a', 'b', 'c'), 'store->_handle_other_method(main DataStore::CAS::Handle a b c)', 'autoload methods' );
